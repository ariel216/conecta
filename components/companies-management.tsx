...
export function CompaniesManagement({
  ...
  const filteredEmpresas = empresas.filter((empresa) => {
    ...
    const matchesDepartment = filterDepartment === "all" || empresa.departamento === filterDepartment
  ...
  const exportToCSV = () => {
    const headers = ["ID", "Código", "Nombre", "Rubro", "Tipo", "Departamento", "Dirección", "Sitio Web", "Facebook"]
          ...
          <CardHeader className="pb-3">
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <CardTitle className="text-lg line-clamp-2">{empresa.nombre}</CardTitle>
                <CardDescription className="flex items-center gap-1 mt-1">
                  <Building2 className="h-4 w-4" />
                  {empresa.codigo}
                </CardDescription>
              </div>
              <Badge variant={empresa.tipo === "comprador" ? "default" : "secondary"} className="ml-2">
                {empresa.tipo}
              </Badge>
            </div>
          </CardHeader>
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  <DropdownMenuItem onClick={() => console.log("Ver empresa", empresa.id)}>
                  ...
                  <DropdownMenuItem onClick={() => onDeleteCompany?.(empresa.id)} className="text-destructive">
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>Administre todas las empresas registradas en el sistema</CardDescription>
            </div>
                  ...
                  {filteredEmpresas.length > 0 ? (
                      ...
                      <TableRow key={empresa.id}>
                        <TableCell>
                          <div>
                            ...
                            <div className="text-sm text-muted-foreground">{empresa.codigo}</div>
                          </div>
                        </TableCell>
                        <TableCell>
                          <Badge variant="outline">{empresa.rubro}</Badge>
                        ...
                        <TableCell>
                          <Badge variant={empresa.tipo === "comprador" ? "default" : "secondary"}>{empresa.tipo}</Badge>
                        </TableCell>
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <MapPin className="h-4 w-4 text-muted-foreground" />
                            <div>
                              <div className="text-sm">{empresa.departamento}</div>
                              <div className="text-xs text-muted-foreground">{empresa.direccion}</div>
                            </div>
                          </div>
                        </TableCell>
                        <TableCell>
                    ...
                    <TableRow>
                      <TableCell colSpan={6} className="text-center py-8 text-muted-foreground">
                        {searchTerm || filterType !== "all" || filterRubro !== "all" || filterDepartment !== "all"
